...
async def run():
    ...
    ### 드론의 (앞,오른,아래,yaw) 방향으로의 속도 0
    ### yaw에 값이 있으면 원운동할 거같음
    ...
    try:
        ...
        return
    ### offboard가 돼야 미세한 조정 가능하다고 생각하면 될 듯
    ...
    await drone.action.goto_location(home_lat, home_lon, float('nan'), float('nan'))
    ### 아까 설정한 home의 gps값 위치로 이동
    async for position in drone.telemetry.position():
        lat = position.latitude_deg
        lon = position.longitude_deg
        print(f"latitude : {lat} / longitude : {lon}")
        if (home_lat - 0.00005 <= lat <= home_lat + 0.00005) and (
                # 오차범위를 주고, 목적지에 도착했는지 판단
                home_lon - 0.00005 <= lon <= home_lon + 0.00005):
            await drone.action.goto_location(home_lat, home_lon, float('nan'), 0)
            print(position)
            print("도착!!!!")
            #목적지에 도착하면, YAW를 0도(북쪽)으로  변경
            break
    ...
    ### 여기에 YOLO 조건 맞으면 곳으로 이동해서 착륙하는 코드 있으면 될 거같다
